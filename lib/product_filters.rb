  ...
  module Core
    module ProductFilters
      ...
      Spree::Product.add_search_scope :size_any do |*opts|
        option_values = []
        opts.each {|x| option_values << Spree::OptionValue.where(presentation: x).pluck(:id).uniq.map(&:to_i) }
        conds = "#{Product.table_name}.id in (select product_id from #{Variant.table_name} v left join spree_option_values_variants ov on ov.variant_id = v.id and v.deleted_at is null where ov.option_value_id in (?))", option_values.flatten
      ...
      def ProductFilters.size_filter
        ...
        sizes = Spree::OptionValue.where(:option_type_id => Spree::OptionType.find_by!(name: "talla")).order("position").map(&:presentation).compact.uniq rescue return
        { 
          name: :size,
          scope: :size_any,
          conds: nil,
      ...
      def ProductFilters.color_filter
        ...
        colors = Spree::OptionValue.where(:option_type_id => Spree::OptionType.find_by!(name: "color")).order("position").map(&:presentation).compact.uniq rescue return
        { 
          name: :colour,
          scope: :color_any,
          conds: nil,
      ...
      def ProductFilters.all_taxons
        taxons = Spree::Taxonomy.all.distinct(:name).map { |t| [t.root] + t.root.descendants.map { |t| t if t.available? } if t.root.available? }.flatten.compact
        {
          name:   :all_taxons,
          scope:  :in_taxons,
